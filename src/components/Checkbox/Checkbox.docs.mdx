...
> `Checkbox` must be wrapped with `useCheckbox` to make the element interactive. But be aware `useCheckbox` hook provides basic checkbox logic only such as handling `isChecked` prop, for more info see [known bugs section](#known-bugs)
